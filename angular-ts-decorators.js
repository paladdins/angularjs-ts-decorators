...
var PlatformRef = {
    bootstrapModule: function (moduleType, compilerOptions) {
        ...
        element(document).ready(function () {
            bootstrap(document, [moduleType.module.name], { strictDi: strictDi });
        });
    }
};
...
function Component(_a) {
    var selector = _a.selector, options = __rest(_a, ["selector"]);
...
function getHooksOnCtrlClass(ctrlClass) {
    return Object.keys(ngLifecycleHooksMap)
        .filter(function (hook) { return isFunction(ctrlClass[hook]); });
}
...
function NgModule(_a) {
    var run = _a.run, config = _a.config, id = _a.id, name = _a.name, _b = _a.declarations, declarations = _b === void 0 ? [] : _b, _c = _a.imports, imports = _c === void 0 ? [] : _c, _d = _a.providers, providers = _d === void 0 ? [] : _d;
    return function (Class) {
        // module registration
...
export { platformBrowserDynamic, Component, Directive, Injectable, Pipe, Input, Output, NgModule, NgModule as NgModuleDecorated, HostListener, ngLifecycleHooksMap, SimpleChange };
